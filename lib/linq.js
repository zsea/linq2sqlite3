...
function Linq(dbName) {
    ...
    function resolveWhereExpressionTree(tree, not, params) {
        ...
        if (tree.operator) {
            var left, right;
            if (tree.left) {
                left = resolveWhereExpressionTree(tree.left, false, params);
            }
            right = resolveWhereExpressionTree(tree.right, tree.operator === "not", params);
            if (left) {
                if (not) {
            ...
            for (var i = 0; i < tree.value.length; i++) {
                values.push(resolveWhereExpressionTree(tree.value[i], false, params))
            }
            return `(${values.join(',')})`;
        }
        else if (tree.type == "expression") {
            return `(${resolveWhereExpressionTree(tree.value, false, params)})`
        }
     ...
     * @returns 
     */
    function resolveWhereItem(lambda, consts, useAlias, params) {
        if (!params) {
            throw new ReferenceError("params can't null or undefined");
        }
        ...
        return resolveWhereExpressionTree(tree, false, params);
    }
    ...
    function resolveWhereNotAlias(lambda, consts, params) {
        if (!params) {
            throw new ReferenceError("params can't null or undefined");
        }
        var data = getEntry(lambda);
     ...
     * @returns 
     */
    function resolveSet(lambda, consts, params) {
        if (!params) {
            throw new ReferenceError("params can't null or undefined");
        }
        var data = getEntry(lambda);
        ...
        if (Array.isArray(tree)) {
            ...
            return tree.map(function (t) {
                return resolveWhereExpressionTree(t, false, params);
            })
        }
        else {
            return [resolveWhereExpressionTree(tree, false, params)];
    ...
    function resolveWhere(useAlias, params) {
        ...
        return wheres.map(function (w) {
            if (Array.isArray(w)) {
                return `(${resolveWhereItem(w[0], w[1], useAlias, params)})`;
            }
            else {
                var _wheres = [];
                for (var field in w) {
    ...
    this.toUpdateSql = function (updater, consts) {
        ...
        if (typeof updater === "object") {
            ...
            sets = resolveSet(updater, consts, params);
        }
        if (sets.length) {
            sql.push(sets.join(','));
        }
    ...
    this.toInsertSql = function (inserter, replacer, consts) {
            ...
            if (replacer) {
                ...
                if (typeof replacer === "object") {
                    for (var field in replacer) {
                        sets.push(`${expression.formater(field)}=${escaper(replacer[field], params)}`)
                    }
                }
                else if (typeof replacer === "function") {
                    sets = resolveSet(replacer, consts, params);
                }
                // if (sets.length) {
                //     sql.push(sets.join(','));
                // }
        ...
        if (replacer && sets.length) {
            sql.push(`ON CONFLICT DO UPDATE SET`);
            sql.push(sets.join(','))
        }
        return {
            sql: sql.join(' '),
        ...
        for (var i = 0; i < tables.length; i++) {
            ...
            if (t.type == "from") {
                ...
                if (t.on) {
                    ...
                    var exps = resolveWhereItem(on_lambda, on_const, true, params);
